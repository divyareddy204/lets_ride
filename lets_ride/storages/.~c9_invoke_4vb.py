...
class StorageImplementation(PostStorageInterface):
    ...
    def get_matching_ride_requests_dto(self, user_id:int):
        ...
        share_object=ShareRide.objects.get(user_id=user_id)
        RideRequest.objects.filter(
            Q(share_object_source=source),
            Q(share_object.destinatiom=destination) | Q(share_object.datetime=dateme,share_object)
            )
